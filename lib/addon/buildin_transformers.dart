...
class ThreeDTransformer extends PageTransformer {
  ...
  Widget transform(Widget child, TransformInfo info) {
    ...
    return Transform(
      transform: Matrix4.identity()..rotate(Vector3(0.0, 2.0, 0.0), position * 1.5),
...
class ZoomOutPageTransformer extends PageTransformer {
  ...
  Widget transform(Widget child, TransformInfo info) {
    ...
    if (position < -1) {
      ...
      final opacity = minAlpha + (scaleFactor - minScale) / (1 - minScale) * (1 - minAlpha);
...
class ScaleAndFadeTransformer extends PageTransformer {
  ...
  ScaleAndFadeTransformer({double fade = 1, double scale = 1})
      : _fade = fade,
        _scale = scale;
